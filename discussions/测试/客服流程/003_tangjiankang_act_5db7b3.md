...
type: act
author: tangjiankang
created: '2026-05-07T18:15:19+08:00'
body_source: manual
index_state: indexed
---
# Pivot 附件与正文图片功能 — 实施方案

> 基于 [`discussions/Pivot/Pivot 附件与正文图片功能需求与设计/004_zhangbo_think_8b00a5.md`](../../var/git/data-test/discussions/Pivot/Pivot%20%E9%99%84%E4%BB%B6%E4%B8%8E%E6%AD%A3%E6%96%87%E5%9B%BE%E7%89%87%E5%8A%9F%E8%83%BD%E9%9C%80%E6%B1%82%E4%B8%8E%E8%AE%BE%E8%AE%A1/004_zhangbo_think_8b00a5.md) 的"完整方案最新版"，对照当前代码核对后给出的可落地实施清单。
...
| 设计假设 | 现状 | 备注 |
|---|---|---|
| `server/db.py` 的 `SCHEMA` 末尾加 `attachments` 表 | ✅ [server/db.py:10-397](../../server/db.py#L10-L397) 是单一 SCHEMA 字符串，新表加在末尾即可 | 无需写 `_migrate` 分支（全新表） |
| 3 个 publish 函数（create / append / comment）末尾加 bind 钩子 | ⚠️ 实际是 6 个函数：`publish_matter_create` / `publish_matter_append` / `publish_matter_mention` / `publish_matter_annotation` / `publish_matter_owner_change` / `publish_matter_event`。设计里说的 "comment" 对应代码中的 `publish_matter_mention`；`publish_matter_annotation` 也有 body 但设计未提及 | 建议把 annotation 也加入 bind 钩子（一致性 + 保险），见 §3 |
| `_render_item` 注入 attachments | ✅ [server/api/matters.py:1342](../../server/api/matters.py#L1342)，分支干净（owner_change / invalidation 早返） | 在 file 分支末尾追加 `out["attachments"] = repo.list_for_file(matter_id, file_rel)` |
| `<DATA_DIR>/attachments/...` 不在 git 仓库下 | ✅ git 在 `<DATA_DIR>/git/<repo>`（[server/app.py:133](../../server/app.py#L133)），attachments 与 git 同级 | 无需改 .gitignore |
| MCP 增 `read_attachment` 工具 | ✅ [server/mcp/tools.py](../../server/mcp/tools.py) + [server/mcp/instructions.py](../../server/mcp/instructions.py) 注册新工具 | instructions 里能力清单要从 9 个加到 10 个 |
...
| **P3 publish bind 钩子** | `publish_matter_create` / `publish_matter_append` / `publish_matter_mention` 末尾扫 body 调 `bind_to_file` | 端到端：草稿态附件未绑定 → 发布后 file_path 已绑 |
...
### 2.1 数据库表（加在 [server/db.py:397](../../server/db.py#L397) `SCHEMA` 字符串末尾，`"""` 之前）
...
落盘根目录：`cfg.data_dir / "attachments"`，[server/app.py:92](../../server/app.py#L92) 创建 repo 后注入 router（与现有 `DraftRepo` 同模式）。
...
依赖加进 [pyproject.toml](../../pyproject.toml)：`pypdf`、`python-docx`、`openpyxl`、`python-pptx`。

### 2.4 配置参数（[server/config.py:11](../../server/config.py#L11)）
...
在 [server/publish.py](../../server/publish.py) 三个写函数 **`write_session` 退出之后** 加扫描钩子：
...
|---|---|---|
| `publish_matter_create`（[publish.py:699](../../server/publish.py#L699)） | `md_body` | `file_rel`（已经在函数里算好） |
| `publish_matter_append`（[publish.py:873](../../server/publish.py#L873)） | `md_body` | `file_rel` |
| `publish_matter_mention`（[publish.py:1010](../../server/publish.py#L1010)，对应设计中的 "comment"） | `body` 参数 | `target_file` |
| **`publish_matter_annotation`**（[publish.py:1120](../../server/publish.py#L1120)，**设计未提**） | `body` 参数 | `target_file` |
...
### 5.1 [server/api/matters.py:1342 _render_item](../../server/api/matters.py#L1342)
...
### 5.2 [server/api/ai.py](../../server/api/ai.py)
...
### 6.1 [server/mcp/tools.py](../../server/mcp/tools.py) 新增
...
`Schema` in [server/mcp/schemas.py](../../server/mcp/schemas.py)：`ReadAttachmentIn { id: str }`、`ReadAttachmentOut { kind, content_type?, text?, base64?, total_chars?, truncated? }`。

### 6.2 [server/mcp/instructions.py:31](../../server/mcp/instructions.py#L31)
...
### 6.3 [server/mcp/server.py](../../server/mcp/server.py) 工具注册
...
主要交互：
1. **按钮 / 拖拽 / 粘贴**三入口都走同一个 `uploadAttachment`；图片走 `usage=inline_image` 默认插入光标位置 `markdown_ref`，其它走 `usage=attachment` 仅挂 chip。
2. 上传成功后调 `onUploaded`，编辑器在 cursor 位置插入 `markdown_ref`（图片）或不插入（普通附件）。
3. 列表渲染：每行 `[icon][filename][size][description?] [删除]`。删除任意一项 → DELETE 接口 + 本地 state 移除 + 同步把 body 中所有 `attachment://<id>` 行删掉（用一个简单的 regex `^.*attachment://${id}.*$`，整行去掉，避免空 alt 残留）。

### 7.2 [web/src/components/matter/CreateFileDialog.tsx](../../web/src/components/matter/CreateFileDialog.tsx)

`BodyMarkdownEditor` 包一层 div，下方挂 `<AttachmentUploader>`；`onPaste` 先尝试图片 → 上传 → 插入 `markdown_ref`，否则 native paste。

### 7.3 [web/src/pages/NewMatter.tsx](../../web/src/pages/NewMatter.tsx)

注意：新建 matter 时 `matter_id` 还不存在！设计里的"草稿态自由删改"前端对此未明说。两种处理：

- **选项 A**：新建 matter 流程的附件先用一个临时 `draft_id`（前端 uuid），上传 API 改成 `POST /api/matters/_draft/{draft_id}/attachments`，提交 matter 时把 `draft_id` 一起 POST 给 `POST /api/matters`，后端把 attachments 的 `matter_id` 重写为新创建的 matter_id 后再 bind。需要 `attachments` 表 `matter_id` 列允许临时值，加一个 `draft_id` 列做映射。
- **选项 B（推荐）**：新建 matter 这一类草稿态**不开附件**，仅在已存在的 matter 里 append 时支持附件。在 v1 这能覆盖 90% 场景。

> ⚠️ 这是设计文档里没明确的一个空档，**建议在动手前找 zhangbo 确认走 A 还是 B**。倾向 B（v1 简化，UX 损失小：要附件就先建空 matter 再传），减少一条临时存储路径。

### 7.4 [web/src/api.ts](../../web/src/api.ts)
...
## 8. P7 — 前端渲染（[web/src/components/matter/FileCard.tsx](../../web/src/components/matter/FileCard.tsx)）

### 8.1 markdownComponents（[FileCard.tsx:50](../../web/src/components/matter/FileCard.tsx#L50)）
...
- `server/tests/test_attachments.py`
  - 上传：白名单、size、同名、数量；状态码 413/422/409
  - 下载/预览：ACL 拒绝（其他 matter 用户）、invalidated 帖子 → 409、preview vs download Content-Disposition
  - 草稿态删除：uploader / 非 uploader / 已绑定
  - bind：`publish_matter_create` 内嵌 `attachment://att_x` → file_path 已绑
- `server/tests/test_attachments_parsing.py`
  - 各类型解析，截断阈值
- `server/tests/test_mcp_read_attachment.py`
  - 文档类截断、图片类 base64
- `web/src/components/matter/__tests__/AttachmentUploader.test.tsx`（如果项目用 vitest）

文档：在 [AI-docs/designs/](../designs/) 下加 `2026-05-XX-attachments-design.md`，把 zhangbo 这篇 think 复制 + 实现细节备注（draft_id、annotation bind 等）落库 — 与现有 [2026-04-29-matter-owner-design.md](../designs/2026-04-29-matter-owner-design.md) 等同款。

---

## 10. 需要先与 zhangbo 对齐的 4 个空档

1. **新建 matter 的草稿态附件路径**（§7.3 选项 A vs B）— 影响有没有 `draft_id` 这条临时通路
2. **`publish_matter_annotation` 是否也加 bind 钩子** — 设计漏了，但行为一致更合理
3. **数量上限的精确语义** — 草稿态 unbound + 已绑定的合算口径，还是简化为"uploader 在 matter 下 unbound ≤ 3"
4. **MCP `read_attachment` 是否对 unauth 用户也走 ACL** — 当前 MCP 用 PAT 解出 user，建议复用同一套 matter ACL
