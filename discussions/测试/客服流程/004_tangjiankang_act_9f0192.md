...
type: act
author: tangjiankang
created: '2026-05-07T18:25:36+08:00'
body_source: manual
index_state: indexed
---
...
| Phase | 范围 |
|---|---|
| **P1 后端存储层** | DB schema + AttachmentRepo + 本地 IO + 配置 |
| **P2 上传/下载/预览 API** | `server/api/attachments.py` 三条路由，**还不接 publish** |
| **P3 publish bind 钩子** | `publish_matter_create` / `publish_matter_append` / `publish_matter_mention` 末尾扫 body 调 `bind_to_file` |
| **P4 _render_item 透传 + AI prompt** | matters.py + ai.py 元数据透传 |
| **P5 MCP `read_attachment`** | tools.py + instructions.py + 文档/图片解析 |
| **P6 前端编辑器** | AttachmentUploader + CreateFileDialog + NewMatter |
| **P7 前端渲染** | FileCard：attachment:// img/a 拦截 + 附件列表区 |

P1 + P2 完成即可 curl 验证后端，前端不必阻塞。
